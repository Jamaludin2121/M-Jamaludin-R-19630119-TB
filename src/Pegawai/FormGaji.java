...
//            rs = st.executeQuery("SELECT * FROM tb_karyawan LEFT JOIN FormDataGaji ON tb_karyawan.kd_karyawan = FormDataGaji.kd_karyawan where id_bulan = '"+bulan+"'");
//            int no = 0;
//            while (rs.next()) {
//                no++;
...
//            tabmodel.fireTableDataChanged();
//            st = cn.createStatement();
//            rs = st.executeQuery("SELECT * FROM tb_karyawan LEFT JOIN FormDataGaji ON tb_karyawan.kd_karyawan = FormDataGaji.kd_karyawan");
//            int no = 0;
//            while (rs.next()) {
//                no++;
////                String status = "";      
...
//        bulan.setSelectedItem("Mei");
    ...
    private void bgajiActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bgajiActionPerformed
        try {
            if (gaji.getText().equals("")) {
                JOptionPane.showMessageDialog(null, "Pilih Karyawan");
            }else{
                st = cn.createStatement();
            ...
            //            st = cn.createStatement();
            //            st.executeUpdate("insert into FormDataGaji (kd_karyawan,tanggal,id_bulan) values('"+lbl_kode.getText()+"','"+dtf2.format(now)+"','"+isibulan.getText()+"')");
            //            JOptionPane.showMessageDialog(null, "Success!");
        } catch (Exception a) {
            JOptionPane.showMessageDialog(null, a.getMessage());
        }
    ...
    private void bgaji1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bgaji1ActionPerformed
        FormDataGaji gaji = new FormDataGaji(this, rootPaneCheckingEnabled);
